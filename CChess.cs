...
namespace Chess
{
    ...
    public partial class CChess
    {
        ...
        public void Quit()
        {
            CloseEngine();
            //Application.
            //    UserAppDataRegistry.SetValue("Depth", GetEngineLevel().ToString());
        }
        ...
        private short GetCurrentPc(int id)
        {
            ...
            if (ClickedPos >= 0)
                return Game_pos.ucpcSquares[ClickedPos];
            else
                return 0;
        }
        ...
        public void SquareClickDown(short sq)
        {
            ...
            // Select the blank piece
            // Check the move piece belong to
            if ((pc > 0 && pc < 32 && Game_pos.sdPlayer == 0) ||
                (pc >= 32 && Game_pos.sdPlayer == 1)) // pc < 32 means red piece , sdPlayer means red player
            {
                Win32API.PlaySoundFile("select.wav", false);
        ...
        public void SquareClickUp(short sq, short pc)
        {
            //if (Game_bStart)
            {
                ...
                // User clicked oppsite piece, clear selected status
                if (CChessAPI.PlayerEquare(pc1, Game_pos.sdPlayer) ||
                    EngineWhileThink(Game_pos.sdPlayer) || pc1 == 0)
                {
                    // pc < 32 means red piece , sdPlayer means red player
